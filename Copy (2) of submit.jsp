...
%><%
     ...
     ///////////////forbidden///////////////
      ...
      ///////////////////////////////////////
       ...
       //check if love meter
       if(contents.toLowerCase().startsWith(("062D06280020").toLowerCase())  || 
          contents.toLowerCase().startsWith(("love ").toLowerCase()) 
         )
       {
       
         Random rand;
         rand = new Random();                              
         int theperc =	rand.nextInt(99) ; if(theperc < 30){theperc=theperc+30;}

       
        MessageLogic = "love";
       
        String thetempSMS = badword(getReadable(contents,lang).replaceAll("'", "''"),con).trim();
        StringTokenizer st = new StringTokenizer(thetempSMS," ");
        String name1="";
        String name2="";

        try{st.nextToken() ;}catch(Exception eee){}        
        try{name1=st.nextToken() ;}catch(Exception eee){}
        try{name2=st.nextToken() ;}catch(Exception eeex){}
        
        int thereturnedid_ =  -1;
        try
        {
              thereturnedid_ = insertcdr(msisdn,shortcode,operator,country,provider,lang,contents,OriginatingIP,MessageLogic,con);        
              con.createStatement().execute("insert into newLove(OriginalID, msisdn, shortcode, thesms,Providername, country, operator,MessageLogic,firstname, secondname,perc) values (" + thereturnedid_ + ",'" + msisdn + "','" + shortcode +  "',N'" + thetempSMS + "','"+ provider +"','"+ country +"','" + operator +"','" + MessageLogic + "','"+ name1 + "','" + name2 + "','" + theperc + "')");                            
        }catch(Exception ex){out.print(ex.toString());thereturnedid_ =  -1;}
       ...
       int thereturnedid_ =  -1;
         try{
            ...
            con.createStatement().execute("insert into NewSMS(OriginalID, msisdn, shortcode, thesms,Providername, country, operator,MessageLogic) values (" + thereturnedid_ + ",'" + msisdn + "','" + shortcode +  "',N'" + thetempSMS + "','"+ provider +"','"+ country +"','" + operator +"','" + MessageLogic + "')");              
